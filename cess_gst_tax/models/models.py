...
class ResPartner(models.Model):
    ...
    state_code=fields.Char(string="State Code",required=True,default='32')
...
class AccountInvoiceLine(models.Model):
    ...
    @api.depends('price_unit', 'discount', 'invoice_line_tax_ids', 'quantity',
                 'product_id', 'invoice_id.partner_id', 'invoice_id.currency_id','cess_tax_id','invoice_id.include',
                 'scheme_discount_amount','scheme_discount')
    def _compute_price_tax(self):
        ...
        if self.invoice_id.type in ['in_invoice', 'in_refund'] and self.invoice_id.include == True:
            if self.cess_tax_id:
                ...
                unit_taxes = self.invoice_line_tax_ids.compute_cess_tax_include(price, currency, q, product=self.product_id,
                ...
                taxes = self.invoice_line_tax_ids.compute_all_include(price,currency, self.quantity,
                                                                      product=self.product_id,
                                                                      partner=self.invoice_id.partner_id)
                unit_taxes = self.invoice_line_tax_ids.compute_all_include(price, currency, q, product=self.product_id,
                                                                   partner=self.invoice_id.partner_id)
            ...
            if self.cess_tax_id:
                taxes = self.invoice_line_tax_ids.compute_cess_tax(price, currency, self.quantity, product=self.product_id,
                                                          partner=self.invoice_id.partner_id,  cess_tax_id = self.cess_tax_id)
                unit_taxes = self.invoice_line_tax_ids.compute_cess_tax(price, currency, q, product=self.product_id,
                                                                   partner=self.invoice_id.partner_id,cess_tax_id = self.cess_tax_id)
            else:
