...
class ConsolidatedXls(ReportXlsx):
    ...
    def get_all_branch(self,data,stock_loc):
        ...
        payment_cash = '''
            ...
            where aj.type='cash' and aj.used_for_rounding is not true and ap.state!='draft'
             and ap.payment_type='inbound' and aj.company_id = %s
        ...
        pos_credit_query = '''
            ...
            and (po.id not in(
            -- select pos_statement_id from account_bank_statement_line where pos_statement_id is not null
            select pos_statement_id from account_bank_statement_line absl 
            left join account_bank_statement abs on abs.id =absl.statement_id where pos_statement_id is not null
             and abs.pos_session_id in 
             (select pos_session.id from pos_session left join pos_config as pc on pc.id =pos_session.config_id 
              where pc.stock_location_id = %s and pc.company_id = %s
            and to_char(date_trunc('day',pos_session.start_at),'YYYY-MM-DD')::date = %s
            and to_char(date_trunc('day',pos_session.stop_at),'YYYY-MM-DD')::date = %s
             ) and abs.company_id =%s

            ))
                '''
        self.env.cr.execute(pos_credit_query, (stock_loc,company_id,date_start,date_start,
                                               stock_loc, company_id, date_start, date_start,
                                                company_id))
        for row in self.env.cr.dictfetchall():
        ...
        pos_credit_query2 = '''
            ...
            and po.invoice_remark is not null
            and ai.amount_total != (
            -- select sum(amount) from account_bank_statement_line where pos_statement_id = po.id
            select sum(absl.amount) from account_bank_statement_line absl 
            left join account_bank_statement abs on abs.id =absl.statement_id where pos_statement_id = po.id
             and abs.pos_session_id in 
             (select pos_session.id from pos_session left join pos_config as pc on pc.id =pos_session.config_id 
              where pc.stock_location_id = %s and pc.company_id = %s
            and to_char(date_trunc('day',pos_session.start_at),'YYYY-MM-DD')::date = %s
    ...
    def get_session_wise(self,data,session):
        ...
        pos_credit_query = '''
            ...
            and po.invoice_remark is not null
            and (po.id not in(select pos_statement_id from account_bank_statement_line absl 
            left join account_bank_statement abs on abs.id =absl.statement_id where pos_statement_id is not null
             and abs.pos_session_id = %s and abs.company_id =%s))

                '''
        self.env.cr.execute(pos_credit_query, (session,company_id,session,company_id))
        for row in self.env.cr.dictfetchall():
            credit += row['credit'] if row['credit'] else 0

        pos_credit_query2 = '''
            select (sum(ai.amount_total)-(select sum(absl.amount) from account_bank_statement_line absl 
             left join account_bank_statement abs on abs.id =absl.statement_id where pos_statement_id = po.id
             and abs.pos_session_id = %s and abs.company_id =%s)) as credit,po.id from account_invoice as ai
            left join pos_order as po on po.invoice_id = ai.id
            where ai.id in(
            select invoice_id from pos_order where session_id = %s and company_id = %s
            and invoice_remark is not null
            ...
            (ai.amount_total != (select sum(absl.amount) from account_bank_statement_line absl 
             left join account_bank_statement abs on abs.id =absl.statement_id where pos_statement_id = po.id
             and abs.pos_session_id = %s and abs.company_id =%s))
            group by po.id
            '''
        self.env.cr.execute(pos_credit_query2, (session, company_id,session, company_id,session, company_id))
        for row in self.env.cr.dictfetchall():
            credit += row['credit'] if row['credit'] else 0
...
ConsolidatedXls('report.consolidated_sale_report.sale_xls.xlsx', 'account.move')
