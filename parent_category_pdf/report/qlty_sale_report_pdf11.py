import time
from odoo import api, models, _
from datetime import datetime


class Reportsales(models.AbstractModel):
    _name = 'report.qlty_salesreport_xls.qlty_salesreport_pdf'

    def get_cash(self, data):

        lines = []
        invoice_id = []

        date_start = data['form']['date_start']
        date_end = data['form']['date_end']
        company_id = data['form']['company_id']
        counter_only = data['form']['counter_only']
        pos_config_ids = data['form']['pos_config_ids']

        cr=0

        if counter_only:
            query2 = '''

                        select pc.id as id ,sum(ap.amount) as payment_amount from account_payment as ap
            			left join account_journal as aj on(aj.id=ap.journal_id)
            			left join pos_config as pc on(pc.id=ap.pos_counter_ids)

            			where aj.type='cash' and ap.state='posted' and ap.payment_type='inbound' and aj.company_id = %s
            			 and  to_char(date_trunc('day',ap.payment_date),'YYYY-MM-DD')
            			 ::date between %s and %s group by pc.id




                        '''

            self.env.cr.execute(query2, (company_id, date_start, date_end,))
            for row in self.env.cr.dictfetchall():
                counters_id = (row['id']) if (row['id']) else 0
                payment_amount = (row['payment_amount']) if (row['payment_amount']) else 0.0

            config = self.env['pos.config'].browse(pos_config_ids)
            # for i in pos_config_ids:
            for pfg in config:
                if pfg.id == counters_id:

                    cr = payment_amount
                else:
                    cr = 0
            cash = 0
            sessions_ids = self.env['pos.session'].search([

                ('config_id', 'in', pos_config_ids)])
            pos_order = self.env['pos.order'].search([('date_order', '>=', date_start), ('date_order', '<=', date_end),
                                                      ('company_id', '=', company_id),
                                                      ('state', 'in', ['paid', 'invoiced', 'done']),
                                                      ('session_id', 'in', sessions_ids.ids)])

            for order in pos_order:
                pos_account = self.env['account.bank.statement.line'].search([('id', 'in', order.statement_ids.ids),('journal_id.name', '!=', 'Round off')])
                for account in pos_account:
                    if account.journal_id.type == 'cash':
                        cash += account.amount

        else:

            cr = 0

            query3 = '''

                                                          select sum(ap.amount) as payment_amount from account_payment as ap
                                                			left join account_journal as aj on(aj.id=ap.journal_id)
                                                			left join pos_config as pc on(pc.id=ap.pos_counter_ids)

                                                			where aj.type='cash' and aj.name<>'Round off' and ap.state='posted' and ap.payment_type='inbound' and aj.company_id = %s
                                                			 and  to_char(date_trunc('day',ap.payment_date),'YYYY-MM-DD')
                                                			 ::date between %s and %s


                                                            '''

            self.env.cr.execute(query3, (company_id, date_start, date_end,))
            for row in self.env.cr.dictfetchall():
                pay_amount = row['payment_amount'] if row['payment_amount'] else 0.0

            query3 = '''

                                                   select sum(d.amount) as amount from(select ps.amount from pos_order as po 
            			 left join pos_session as pos on (pos.id=po.session_id)
            			 left join account_bank_statement as psl on (psl.pos_session_id=pos.id)
            			 left join account_bank_statement_line as ps on (ps.statement_id=psl.id)
            			left join account_journal as aj on(aj.id=ps.journal_id)
            			where aj.type='cash' and aj.name<>'Round off' and aj.company_id=%s and ps.amount>0
            			 and  to_char(date_trunc('day',po.date_order),'YYYY-MM-DD')
            			 ::date between %s and %s group by ps.id)as d
                                                                        '''

            self.env.cr.execute(query3, (company_id, date_start, date_end,))
            for row in self.env.cr.dictfetchall():
                cash_am = row['amount'] if row['amount'] else 0.0
            query4 = '''
select sum(apr.amount) as alp  from account_partial_reconcile as apr

	left join account_move_line as aml on aml.id=apr.credit_move_id
	left join account_move as am on am.id=aml.move_id
	left join account_journal as aj on aj.id=am.journal_id
	left join account_invoice as a on (a.journal_id=aml.journal_id)



	where aj.company_id=%s and aj.type='cash' and aj.name<>'Round off'
            			 and  to_char(date_trunc('day',apr.create_date),'YYYY-MM-DD')
            			 ::date between %s and %s and aml.payment_id not in ( select ap.id from account_payment as ap
                                                			left join account_journal as aj on(aj.id=ap.journal_id)
                                                			left join pos_config as pc on(pc.id=ap.pos_counter_ids)

                                                			where aj.name<>'Round off' and ap.payment_type='inbound' and aj.company_id = %s
                                                			 and  to_char(date_trunc('day',ap.payment_date),'YYYY-MM-DD')
                                                			 ::date between %s and %s)
                                                                                   '''

            self.env.cr.execute(query4, (company_id, date_start, date_end,company_id, date_start, date_end,))
            for row in self.env.cr.dictfetchall():
                cash_alp = row['alp'] if row['alp'] else 0.0
            cash = cash_am + pay_amount+cash_alp

            # query2 = '''
            #
            #                         select pc.id as id ,sum(ap.amount) as payment_amount from account_payment as ap
            #             			left join account_journal as aj on(aj.id=ap.journal_id)
            #             			left join pos_config as pc on(pc.id=ap.pos_counter_ids)
            #
            #             			where aj.type='cash' and ap.state='posted' and ap.payment_type='inbound' and aj.company_id = %s
            #             			 and  to_char(date_trunc('day',ap.payment_date),'YYYY-MM-DD')
            #             			 ::date between %s and %s group by pc.id
            #
            #
            #
            #
            #                         '''
            #
            # self.env.cr.execute(query2, (company_id, date_start, date_end,))
            # for row in self.env.cr.dictfetchall():
            #     counters_id = (row['id']) if (row['id']) else 0
            #     payment_amount = (row['payment_amount']) if (row['payment_amount']) else 0.0
            #
            #
            #
            #     # cr=payment_amount
            #
            # config = self.env['pos.config'].browse(pos_config_ids)
            # # for i in pos_config_ids:
            # for pfg in config:
            #     if pfg.id:
            #
            #         cr = payment_amount if payment_amount else 0.0
            #     else:
            #         cr = 0
            # cash = 0
            # # sessions_ids = self.env['pos.session'].search([
            # #
            # #     ('config_id', 'in', pos_config_ids)])
            # pos_order = self.env['pos.order'].search([('date_order', '>=', date_start), ('date_order', '<=', date_end),
            #                                           ('company_id', '=', company_id),
            #                                           ('state', 'in', ['paid', 'invoiced', 'done']),
            #                                           ])
            #
            # for order in pos_order:
            #     pos_account = self.env['account.bank.statement.line'].search(
            #         [('id', 'in', order.statement_ids.ids),])
            #     for account in pos_account:
            #         if account.journal_id.type == 'cash':
            #             cash += account.amount

        res = {
            'cash': cash+cr,
        }

        lines.append(res)

        if lines:
            return lines
        else:
            return []

    def get_card(self, data):

        lines = []
        invoice_id = []
        date_start = data['form']['date_start']
        date_end = data['form']['date_end']
        company_id = data['form']['company_id']
        counter_only = data['form']['counter_only']
        pos_config_ids = data['form']['pos_config_ids']


        if counter_only:
            cr = 0

            query5 = '''

                                    select pc.id as id ,sum(ap.amount) as payment_amount from account_payment as ap
                        			left join account_journal as aj on(aj.id=ap.journal_id)
                        			left join pos_config as pc on(pc.id=ap.pos_counter_ids)

                        			where aj.type='bank' and ap.state='posted' and ap.payment_type='inbound' and aj.company_id = %s
                        			 and  to_char(date_trunc('day',ap.payment_date),'YYYY-MM-DD')
                        			 ::date between %s and %s group by pc.id




                                    '''

            self.env.cr.execute(query5, (company_id, date_start, date_end,))
            for row in self.env.cr.dictfetchall():
                counters_id = (row['id']) if (row['id']) else 0
                payment_amount = (row['payment_amount']) if (row['payment_amount']) else 0.0

            config = self.env['pos.config'].browse(pos_config_ids)
            # for i in pos_config_ids:
            for pfg in config:
                if pfg.id == counters_id:

                    cr = payment_amount
                else:
                    cr = 0
            card = 0


            sessions_ids = self.env['pos.session'].search([

                ('config_id', 'in', pos_config_ids)])
            pos_order = self.env['pos.order'].search([('date_order', '>=', date_start), ('date_order', '<=', date_end),
                                                      ('company_id', '=', company_id),
                                                      ('state', 'in', ['paid', 'invoiced', 'done']),
                                                      ('session_id', 'in', sessions_ids.ids)])

            for order in pos_order:
                pos_account = self.env['account.bank.statement.line'].search([('id', 'in', order.statement_ids.ids),('journal_id.name', '!=', 'Round off')])
                for account in pos_account:
                    if account.journal_id.type == 'bank':
                        card += account.amount

        else:

            cr=0

            query6 = '''

                                              select sum(ap.amount) as payment_amount from account_payment as ap
                                    			left join account_journal as aj on(aj.id=ap.journal_id)
                                    			left join pos_config as pc on(pc.id=ap.pos_counter_ids)

                                    			where aj.type='bank' and aj.name<>'Round off' and ap.state='posted' and ap.payment_type='inbound' and aj.company_id = %s
                                    			 and  to_char(date_trunc('day',ap.payment_date),'YYYY-MM-DD')
                                    			 ::date between %s and %s


                                                '''

            self.env.cr.execute(query6, (company_id, date_start, date_end,))
            for row in self.env.cr.dictfetchall():
                pay_amount = row['payment_amount'] if row['payment_amount'] else 0.0

            query7 = '''

                                       select sum(d.amount) as amount from(select ps.amount from pos_order as po 
			 left join pos_session as pos on (pos.id=po.session_id)
			 left join account_bank_statement as psl on (psl.pos_session_id=pos.id)
			 left join account_bank_statement_line as ps on (ps.statement_id=psl.id)
			left join account_journal as aj on(aj.id=ps.journal_id)
			where aj.type='bank' and aj.name<>'Round off' and aj.company_id=%s and ps.amount>0
			 and  to_char(date_trunc('day',po.date_order),'YYYY-MM-DD')
			 ::date between %s and %s group by ps.id)as d
                                                            '''

            self.env.cr.execute(query7, (company_id, date_start, date_end,))
            for row in self.env.cr.dictfetchall():
                card_am = row['amount'] if row['amount'] else 0.0
            query8 = '''
            select sum(apr.amount) as alp  from account_partial_reconcile as apr

            	left join account_move_line as aml on aml.id=apr.credit_move_id
            	left join account_move as am on am.id=aml.move_id
            	left join account_journal as aj on aj.id=am.journal_id
            	left join account_invoice as a on (a.journal_id=aml.journal_id)



            	where aj.company_id=%s and aj.type='bank' and aj.name<>'Round off'
                        			 and  to_char(date_trunc('day',apr.create_date),'YYYY-MM-DD')
                        			 ::date between %s and %s and aml.payment_id not in ( select ap.id from account_payment as ap
                                                            			left join account_journal as aj on(aj.id=ap.journal_id)
                                                            			left join pos_config as pc on(pc.id=ap.pos_counter_ids)

                                                            			where aj.name<>'Round off' and ap.payment_type='inbound' and aj.company_id = %s
                                                            			 and  to_char(date_trunc('day',ap.payment_date),'YYYY-MM-DD')
                                                            			 ::date between %s and %s)
                                                                                               '''

            self.env.cr.execute(query8, (company_id, date_start, date_end, company_id, date_start, date_end,))
            for row in self.env.cr.dictfetchall():
                cash_alp = row['alp'] if row['alp'] else 0.0
            card = card_am+pay_amount+cash_alp

            # card = 0
            # query1 = '''select invoice_id from pos_order where invoice_id is not null and company_id = %s and to_char(date_trunc('day',date_order),'YYYY-MM-DD')::date between %s and %s'''
            # self.env.cr.execute(query1, (company_id, date_start, date_end,))
            # for row in self.env.cr.dictfetchall():
            #     invoice_id.append(row['invoice_id'])
            # pos_order = self.env['pos.order'].search([('date_order', '>=', date_start), ('date_order', '<=', date_end),
            #                                           ('company_id', '=', company_id),
            #                                           ('state', 'in', ['paid', 'invoiced', 'done'])])
            # for order in pos_order:
            #     pos_account = self.env['account.bank.statement.line'].search([('id', 'in', order.statement_ids.ids)])
            #     for account in pos_account:
            #         if account.journal_id.type == 'bank':
            #             card += account.amount
            # account_invoice = self.env['account.invoice'].search(
            #     [('date_invoice', '>=', date_start), ('date_invoice', '<=', date_end),
            #      ('company_id', '=', company_id),
            #      ('state', 'in', ['paid', 'open']), ('type', '=', 'out_invoice'),
            #      ('id', 'in', invoice_id)])
            # for invoice in account_invoice:
            #     for i in invoice.payment_move_line_ids:
            #         account_move_line = self.env['account.move.line'].search(
            #             [('id', '=', i.id), ('journal_id.type', '=', 'bank'), ('move_id.state', '=', 'posted'),('journal_id.name', '!=', 'Round off')])
            #         for j in account_move_line:
            #             card += j.credit

        res = {
            'card': card+cr,
        }

        lines.append(res)

        if lines:
            return lines
        else:
            return []

    def get_credit(self, data):

        lines = []
        # invoice_id = []
        # invoice_remark = []
        # residual = []
        # amount_total = []

        date_start = data['form']['date_start']
        date_end = data['form']['date_end']
        company_id = data['form']['company_id']
        counter_only = data['form']['counter_only']

        pos_config_ids = data['form']['pos_config_ids']

        credit = 0
        cre=0
        am=0
        if counter_only:
            cre = 0
            am = 0
            cash = 0
        #     query1 = '''
        #
        #          SELECT session.config_id as config,
        #         sum(a.residual) as residual ,
        #         sum(a.amount_total) as amount_total
        #
        #   FROM account_invoice as a
        # --left join account_invoice_line as ai on (a.id=ai.invoice_id)
        # left join pos_order as po on(po.invoice_id=a.id)
        #
        # LEFT JOIN pos_session session ON (session.id=po.session_id)
        #  where
        #         po.state in ('paid','done','invoiced') and a.state in ('open','draft')
        #         and a.partial_payment_remark = po.invoice_remark
        #         and po.invoice_id = a.id
        # and session.config_id in %s AND po.company_id= %s
        #         AND to_char(date_trunc('day',po.date_order),'YYYY-MM-DD')::date between %s and %s
        #
        #         group by session.config_id
        #                         '''
        #     self.env.cr.execute(query1, (tuple(pos_config_ids), company_id, date_start, date_end,))
        #     for row in self.env.cr.dictfetchall():
        #         config=(row['config'] if row['config'] else " ")
        #         for i in pos_config_ids:
        #             if config==i:
        #
        #                 credit2=(row['residual'] if row['residual'] else " ")
        #             else:
        #                 credit2=0.0
        #             amount_total=(row['amount_total'] if row['amount_total'] else " ")
        #
        #     res = {
        #
        #         'credit': credit2,
        #
        #             }
        #
        #     lines.append(res)
        #
        #
        #     if lines:
        #         return lines
        #     else:
        #         return []

            invoice_id = []
            invoice_remark = []
            # counters_id = []
            # payment_amount = []
            residual = []
            amount_total = []
            # cr=0
            credit = 0
            cre = 0
            qu1 = '''


                                    select a.id,a.partial_payment_remark,a.amount_total,pc.id config
                                                            from account_invoice as a
                                                            left join pos_config as pc on pc.id=a.pos_config_ids
                                                      where a.partial_payment_remark is not null 
                                                      and a.pos_config_ids in %s and a.company_id = %s 
                                                      and a.state='paid' and a.pos_config_ids is not null
                                                       and to_char(date_trunc('day',a.date_invoice),'YYYY-MM-DD')::date between %s and %s





                                    '''

            self.env.cr.execute(qu1, (tuple(pos_config_ids), company_id, date_start, date_end,))
            for row in self.env.cr.dictfetchall():
                config = (row['config'] if row['config'] else 0.0)

                # for i in pos_config_ids:

                config = self.env['pos.config'].browse(pos_config_ids)
                # for i in pos_config_ids:
                for pfg in config:
                    if pfg.id == config:

                        cre = (row['amount_total'] if row['amount_total'] else 0.0)
                    else:
                        cre = 0


            query31 = '''
                                                        SELECT session.config_id as config,
                                                sum(a.residual) as residual ,
                                                sum(a.amount_total) as amount_total

                                          FROM account_invoice as a
                                		--left join account_invoice_line as ai on (a.id=ai.invoice_id)
                                		left join pos_order as po on(po.invoice_id=a.id)

                                		LEFT JOIN pos_session session ON (session.id=po.session_id)
                                		 where
                                                po.state in ('paid','done','invoiced') and a.state in ('open','draft')
                                                and a.partial_payment_remark = po.invoice_remark
                                                and po.invoice_id = a.id
                                		and session.config_id in %s AND po.company_id= %s
                                                AND to_char(date_trunc('day',po.date_order),'YYYY-MM-DD')::date between %s and %s

                                                group by session.config_id
                                                        '''

            self.env.cr.execute(query31, (tuple(pos_config_ids), company_id, date_start, date_end,))
            for row in self.env.cr.dictfetchall():
                config = (row['config'] if row['config'] else 0.0)

                config = self.env['pos.config'].browse(pos_config_ids)
                # for i in pos_config_ids:
                for pfg in config:
                    if pfg.id == config:

                        am = (row['residual'] if row['residual'] else 0.0)
                    else:
                        am = 0




            # am=0
            #
            # qu = '''
            #
            #
            #             select a.id as id,a.partial_payment_remark as partial_payment_remark,
            #             a.amount_total as amount_total,pc.id as config
            #                                     from account_invoice as a
            #                                     left join pos_config as pc on pc.id=a.pos_config_ids
            #                               where a.partial_payment_remark is not null
            #                               and a.pos_config_ids in %s and a.company_id = %s
            #                               and a.state='paid' and a.pos_config_ids is not null
            #                                and to_char(date_trunc('day',a.date_invoice),'YYYY-MM-DD')::date between %s and %s
            #
            #
            #
            #
            #
            #             '''
            #
            # self.env.cr.execute(qu, (tuple(pos_config_ids), company_id, date_start, date_end,))
            # for row in self.env.cr.dictfetchall():
            #     config = (row['config'] if row['config'] else 0.0)
            #
            #     payment_amount = (row['amount_total']) if (row['amount_total']) else 0.0
            #
            #     config = self.env['pos.config'].browse(pos_config_ids)
            #     # for i in pos_config_ids:
            #     for pfg in config:
            #         if pfg.id == config:
            #
            #             cre = payment_amount
            #         else:
            #             cre = 0
            #
            #     # for i in pos_config_ids:
            #     # if config_id == config:
            #     #     cre = (row['amount_total'] if row['amount_total'] else 0.0)
            # query11 = '''
            #
            #                        select id,partial_payment_remark
            #                         from account_invoice
            #                   where partial_payment_remark is not null and company_id = %s
            #                    and to_char(date_trunc('day',date_invoice),'YYYY-MM-DD')::date between %s and %s
            #                         '''
            # # query1 = '''select invoice_id from pos_order where invoice_id is not null and invoice_remark is not null and company_id = %s and to_char(date_trunc('day',date_order),'YYYY-MM-DD')::date between %s and %s'''
            # self.env.cr.execute(query11, (company_id, date_start, date_end,))
            # for row in self.env.cr.dictfetchall():
            #     invoice_id.append(row['id'])
            #     invoice_remark.append(row['partial_payment_remark'])
            #
            #     # for cu in counters_id:
            #
            #     # for cu in counters_id:
            #
            #
            #
            #
            #
            # sessions_ids = self.env['pos.session'].search([
            #
            #     ('config_id', 'in', pos_config_ids)])
            # pos_order = self.env['pos.order'].search([('date_order', '>=', date_start), ('date_order', '<=', date_end),
            #                                           ('company_id', '=', company_id),
            #                                           ('state', 'in', ['paid', 'invoiced', 'done']),
            #                                           ('session_id', 'in', sessions_ids.ids),
            #                                           ('invoice_id','in',invoice_id),('invoice_remark','in',invoice_remark)])
            #
            #
            # invoice = self.env['account.invoice'].search(
            #     [('date_invoice', '>=', date_start), ('date_invoice', '<=', date_end),
            #      ('company_id', '=', company_id),
            #      ('state', 'in', ['draft', 'open']), ('type', '=', 'out_invoice'),
            #      ])
            #
            #
            # for order in pos_order:
            #     pos_account = self.env['account.bank.statement.line'].search([('id', 'in', order.statement_ids.ids),('journal_id.name', '!=', 'Round off')])
            #     config = self.env['pos.config'].browse(pos_config_ids)
            #     # for i in pos_config_ids:
            #     for pfg in config:
            #         for h in order.config_id:
            #
            #         # for i in pos_config_ids:
            #             if pfg.id==h.id:
            #
            #                 for o in order.invoice_id:
            #                     if o.state == 'draft':
            #                         credit += o.amount_total
            #                     if o.state == 'open':
            #                         credit += o.residual
            #
            #             else:
            #                 credit =0.0


                    # res = {
                    #     'credit': credit,
                    # }
                    #
                    # lines.append(res)
                    #
                    # if lines:
                    #     return lines
                    # else:
                    #     return []



        else:

            invoice_id = []
            invoice_remark = []
            residual = []
            amount_total = []
            am=0
            cre=0

            query21 = '''

                        select sum(apr.amount) as am from account_partial_reconcile as apr

            	left join account_move_line as aml on aml.id=apr.credit_move_id
            	left join account_journal as aj on aj.id=aml.journal_id
            	where  aj.name<>'Round off' and aj.company_id=1 
                        			 and  to_char(date_trunc('day',apr.create_date),'YYYY-MM-DD')
                        			 ::date between %s and %s and aml.payment_id not in ( select ap.id from account_payment as ap
                                                            			left join account_journal as aj on(aj.id=ap.journal_id)
                                                            			left join pos_config as pc on(pc.id=ap.pos_counter_ids)

                                                            			where aj.name<>'Round off' and ap.payment_type='inbound' and aj.company_id = %s
                                                            			 and  to_char(date_trunc('day',ap.payment_date),'YYYY-MM-DD')
                                                            			 ::date between %s and %s)


                        '''

            self.env.cr.execute(query21, (date_start, date_end, company_id, date_start, date_end,))
            for row in self.env.cr.dictfetchall():
                if row['am']:
                    am = (row['am'])
                else:
                    am = 0.0

            query1 = '''
                                    select invoice_id,invoice_remark,picking_id from pos_order 
                                      where invoice_id is not null and invoice_remark is not null and company_id = %s
                                       and to_char(date_trunc('day',date_order),'YYYY-MM-DD')::date between %s and %s
        
                                    '''
            # query1 = '''select invoice_id from pos_order where invoice_id is not null and invoice_remark is not null and company_id = %s and to_char(date_trunc('day',date_order),'YYYY-MM-DD')::date between %s and %s'''
            self.env.cr.execute(query1, (company_id, date_start, date_end,))
            for row in self.env.cr.dictfetchall():
                invoice_id.append(row['invoice_id'])
                invoice_remark.append(row['invoice_remark'])

            invoice = self.env['account.invoice'].search(
                [('date_invoice', '>=', date_start), ('date_invoice', '<=', date_end),
                 ('company_id', '=', company_id),
                 ('state', 'in', ['draft', 'open']), ('type', '=', 'out_invoice'),
                 ('id', 'in', invoice_id), ('partial_payment_remark', 'in', invoice_remark)])
            for order in invoice:
                if order.state == 'draft':
                    credit += order.amount_total
                if order.state == 'open':
                    credit += order.residual


        res = {
            'credit': credit+am+cre if credit else 0.0,
        }

        lines.append(res)

        if lines:
            return lines
        else:
            return []


    def get_credit_so(self, data, config_id):
        lines = []
        # invoice_id = []
        # invoice_remark = []
        # residual = []
        # amount_total = []


        date_start = data['form']['date_start']
        date_end = data['form']['date_end']
        company_id = data['form']['company_id']
        counter_only = data['form']['counter_only']
        pos_config_ids = data['form']['pos_config_ids']

        credit = 0
        cre=0

        if counter_only:
            cash = 0


            qu1='''
            
            
            select a.id,a.partial_payment_remark,a.amount_total,pc.id config
                                    from account_invoice as a
                                    left join pos_config as pc on pc.id=a.pos_config_ids
                              where a.partial_payment_remark is not null 
                              and a.pos_config_ids in %s and a.company_id = %s 
                              and a.state='paid' and a.pos_config_ids is not null
                               and to_char(date_trunc('day',a.date_invoice),'YYYY-MM-DD')::date between %s and %s

            
            
            
            
            '''

            self.env.cr.execute(qu1, (tuple(pos_config_ids), company_id, date_start, date_end,))
            for row in self.env.cr.dictfetchall():
                config = (row['config'] if row['config'] else 0.0)

                # for i in pos_config_ids:
                if config_id == config:
                    cre = (row['amount_total'] if row['amount_total'] else 0.0)


            query31 = '''
                                SELECT session.config_id as config,
                        sum(a.residual) as residual ,
                        sum(a.amount_total) as amount_total

                  FROM account_invoice as a
        		--left join account_invoice_line as ai on (a.id=ai.invoice_id)
        		left join pos_order as po on(po.invoice_id=a.id)

        		LEFT JOIN pos_session session ON (session.id=po.session_id)
        		 where
                        po.state in ('paid','done','invoiced') and a.state in ('open','draft')
                        and a.partial_payment_remark = po.invoice_remark
                        and po.invoice_id = a.id
        		and session.config_id in %s AND po.company_id= %s
                        AND to_char(date_trunc('day',po.date_order),'YYYY-MM-DD')::date between %s and %s

                        group by session.config_id
                                '''

            self.env.cr.execute(query31, (tuple(pos_config_ids), company_id, date_start, date_end,))
            for row in self.env.cr.dictfetchall():
                config = (row['config'] if row['config'] else 0.0)

                # for i in pos_config_ids:
                if config_id == config:

                    credit = (row['residual'] if row['residual'] else 0.0)
                # else:
                #     credit = 0.0


        res = {

                'credit': credit+cre,

            }

        lines.append(res)
        if res:
            return res
        else:
            return []

        # self.env.cr.execute(query1, (tuple(pos_config_ids),company_id, date_start, date_end,))
            # for row in self.env.cr.dictfetchall():
            #     # invoice_id.append(row['invoice_id'])
            #     # invoice_remark.append(row['invoice_remark'])
            #     residual.append(row['residual'])
            #     amount_total.append(row['amount_total'])
            # sessions_ids = self.env['pos.session'].search([
            #
            #     ('config_id', 'in', pos_config_ids)])
            #
            # invoice = self.env['account.invoice'].search(
            #     [('date_invoice', '>=', date_start), ('date_invoice', '<=', date_end),
            #      ('company_id', '=', company_id),
            #      ('state', 'in', ['draft', 'open']), ('type', '=', 'out_invoice'),
            #      ])
            #
            # pos_order = self.env['pos.order'].search([('date_order', '>=', date_start), ('date_order', '<=', date_end),
            #                                           ('company_id', '=', company_id),
            #                                           ('state', 'in', ['paid', 'invoiced', 'done']),
            #
            #                                           ('session_id', 'in', sessions_ids.ids)])
            # for order in invoice:
            #     if order.state == 'draft':
            #         credit = amount_total
            #     if order.state == 'open':
            #         credit = residual

        # if counter_only:
        #     # query1 = '''
        #     # select invoice_id from pos_order where invoice_id is not null and invoice_remark is
        #     # not null and company_id = %s and to_char(date_trunc('day',date_order),'YYYY-MM-DD')::
        #     # date between %s and %s
        #     # '''
        #
        #     query1 ='''
        #
        #
        #     select invoice_id,invoice_remark,picking_id from pos_order
	     #      where invoice_id is not null and invoice_remark is not null and company_id = %s
	     #       and to_char(date_trunc('day',date_order),'YYYY-MM-DD')::date between %s and %s
        #
        #
        #     '''
        #     self.env.cr.execute(query1, (company_id, date_start, date_end,))
        #     for row in self.env.cr.dictfetchall():
        #         invoice_id.append(row['invoice_id'])
        #         invoice_remark.append(row['invoice_remark'])
        #         # config=self.env['pos.config'].search([('name','=','DESHABHIMANI')]).id
        #         # if config==config_id:
        #     credit = 0
        #     invoice = self.env['account.invoice'].search(
        #             [('date_invoice', '>=', date_start), ('date_invoice', '<=', date_end),
        #              ('company_id', '=', company_id),
        #              ('state', 'in', ['draft', 'open']), ('type', '=', 'out_invoice'),
        #              ('id', 'in', invoice_id),('partial_payment_remark','=',invoice_remark)])
        #     for order in invoice:
        #         if order.state == 'draft' :
        #             credit += order.amount_total
        #         if order.state == 'open':
        #             credit += order.residual

        # res = {
        #     'credit': credit if credit else 0.0,
        # }

        # lines.append(res)



    def get_card_so(self, data, config_id):
        lines = []

        date_start = data['form']['date_start']
        date_end = data['form']['date_end']
        company_id = data['form']['company_id']
        counter_only = data['form']['counter_only']
        pos_config_ids = data['form']['pos_config_ids']

        credit = 0

        if counter_only:

            sessions_ids = self.env['pos.session'].search([

                ('config_id', '=', config_id)])
            pos_order = self.env['pos.order'].search([('date_order', '>=', date_start), ('date_order', '<=', date_end),
                                                      ('company_id', '=', company_id),
                                                      ('state', 'in', ['paid', 'invoiced', 'done']),
                                                      ('session_id', 'in', sessions_ids.ids)])

            st_line_ids = self.env["account.bank.statement.line"].search(
                [('pos_statement_id', 'in', pos_order.ids)]).ids
            if st_line_ids:
                self.env.cr.execute("""
                            SELECT COALESCE(sum(amount),'0') total
                FROM account_bank_statement_line AS absl,
                     account_bank_statement AS abs,
                     account_journal AS aj 
                WHERE absl.statement_id = abs.id
                    AND abs.journal_id = aj.id and aj.type ='cash'
                    --and aj.debt = 'true'
                    AND absl.id IN %s 
                        """, (tuple(st_line_ids),))
                credit = self.env.cr.dictfetchall()
            else:
                credit = [{'total': 0.0}]

        res = {
            'credit': credit[0]['total'] if credit else 0.0,
        }

        # lines.append(res)

        if res:
            return res
        else:
            return []

    def get_cash_wise(self, data, config_id):
        lines = []

        date_start = data['form']['date_start']
        date_end = data['form']['date_end']
        company_id = data['form']['company_id']
        counter_only = data['form']['counter_only']
        pos_config_ids = data['form']['pos_config_ids']
        # date_start=
        cash = 0

        cr = 0
        cm = 0

        if counter_only:

            re = '''


                    select a.pos_config_ids as id,sum(apr.amount) as amr from account_partial_reconcile as apr

        	left join account_move_line as aml on aml.id=apr.credit_move_id
        	left join account_move as am on am.id=aml.move_id
        	left join account_journal as aj on aj.id=am.journal_id
        	left join account_account as aa on aml.account_id=aa.id
        		left join account_invoice as a on (a.account_id=aa.id)




        	where aj.company_id=%s and aj.type='cash' and aj.name<>'Round off' and a.partial_payment_remark is not null and a.state='paid'
        	and a.pos_config_ids is not null 
                    			 and  to_char(date_trunc('day',apr.create_date),'YYYY-MM-DD')
                    			 ::date between %s and %s and aml.payment_id not in ( select ap.id from account_payment as ap
                                                        			left join account_journal as aj on(aj.id=ap.journal_id)
                                                        			left join pos_config as pc on(pc.id=ap.pos_counter_ids)

                                                        			where aj.name<>'Round off' and ap.payment_type='inbound' and aj.company_id = %s
                                                        			 and  to_char(date_trunc('day',ap.payment_date),'YYYY-MM-DD')
                                                        			 ::date between %s and %s)

                                                        			 group by a.pos_config_ids






                    '''

            self.env.cr.execute(re, (company_id, date_start, date_end, company_id, date_start, date_end,))
            for row in self.env.cr.dictfetchall():

                coun_id = (row['id'] if row['id'] else 0)

                # for i in pos_config_ids:
                if config_id == coun_id:
                    cm += (row['amr'] if row['amr'] else 0.0)
                else:
                    cm = 0

            query25 = '''

                                                        select pc.id as id ,sum(ap.amount) as payment_amount from account_payment as ap
                                            			left join account_journal as aj on(aj.id=ap.journal_id)
                                            			left join pos_config as pc on(pc.id=ap.pos_counter_ids)

                                            			where aj.type='cash' and ap.state='posted' and ap.payment_type='inbound' and aj.company_id = %s
                                            			 and  to_char(date_trunc('day',ap.payment_date),'YYYY-MM-DD')
                                            			 ::date between %s and %s group by pc.id




                                                        '''

            self.env.cr.execute(query25, (company_id, date_start, date_end,))
            for row in self.env.cr.dictfetchall():

                counters_id = (row['id'] if row['id'] else 0)

                # for i in pos_config_ids:
                if config_id == counters_id:
                    cr += (row['payment_amount'] if row['payment_amount'] else 0.0)
                else:
                    cr = 0

            sessions_ids = self.env['pos.session'].search([

                ('config_id', '=', config_id)])
            pos_order = self.env['pos.order'].search([('date_order', '>=', date_start), ('date_order', '<=', date_end),
                                                      ('company_id', '=', company_id),
                                                      ('state', 'in', ['paid', 'invoiced', 'done']),
                                                      ('session_id', 'in', sessions_ids.ids)])

            for inv in pos_order:
                config = self.env['pos.config'].browse(pos_config_ids)
                # for i in pos_config_ids:
                for pfg in config:
                    if pfg.id == config_id:
                        for invoice in inv.invoice_id:
                            for i in invoice.payment_move_line_ids:
                                account_move_line = self.env['account.move.line'].search(
                                    [('id', '=', i.id), ('journal_id.type', '=', 'cash'),
                                     ('move_id.state', '=', 'posted'), ('journal_id.name', '!=', 'Round off'),
                                     ('name', '!=', 'Customer Payment')])
                                for j in account_move_line:
                                    cash += j.credit

                        # for i in pos_config_ids:

        res = {
            'cash': cash + cr + cm,
        }

        # lines.append(res)

        if res:
            return res
        else:
            return []

        # cash = 0
        # # cr=0
        # # cm=0
        #
        # if counter_only:
        #     cr = 0
        #     cm = 0
        #
        #     re = '''
        #
        #
        #                 select a.pos_config_ids as id,sum(apr.amount) as amr from account_partial_reconcile as apr
        #
        #     	left join account_move_line as aml on aml.id=apr.credit_move_id
        #     	left join account_move as am on am.id=aml.move_id
        #     	left join account_journal as aj on aj.id=am.journal_id
        #     	left join account_account as aa on aml.account_id=aa.id
        #     		left join account_invoice as a on (a.account_id=aa.id)
        #
        #
        #
        #
        #     	where aj.company_id=%s and aj.type='cash' and aj.name<>'Round off' and a.partial_payment_remark is not null and a.state='paid'
        #     	and a.pos_config_ids is not null
        #                 			 and  to_char(date_trunc('day',apr.create_date),'YYYY-MM-DD')
        #                 			 ::date between %s and %s and aml.payment_id not in ( select ap.id from account_payment as ap
        #                                                     			left join account_journal as aj on(aj.id=ap.journal_id)
        #                                                     			left join pos_config as pc on(pc.id=ap.pos_counter_ids)
        #
        #                                                     			where aj.name<>'Round off' and ap.payment_type='inbound' and aj.company_id = %s
        #                                                     			 and  to_char(date_trunc('day',ap.payment_date),'YYYY-MM-DD')
        #                                                     			 ::date between %s and %s)
        #
        #                                                     			 group by a.pos_config_ids
        #
        #
        #
        #
        #
        #
        #                 '''
        #
        #     self.env.cr.execute(re, (company_id, date_start, date_end, company_id, date_start, date_end,))
        #     for row1 in self.env.cr.dictfetchall():
        #         if row1['id']:
        #             coun_id = row1['id']
        #         else:
        #             coun_id =0
        #
        #
        #         # coun_id = (row1['id'] if row1['id'] else 0)
        #
        #         # for i in pos_config_ids:
        #         if config_id == coun_id:
        #             if (row1['amr']):
        #                 cm+=(row1['amr'])
        #             else:
        #                 cm=0
        #             # cm += (row['amr'] if row['amr'] else 0.0)
        #         else:
        #             cm = 0
        #
        #
        #
        #     query2 = '''
        #
        #                             select pc.id as id ,sum(ap.amount) as payment_amount from account_payment as ap
        #                 			left join account_journal as aj on(aj.id=ap.journal_id)
        #                 			left join pos_config as pc on(pc.id=ap.pos_counter_ids)
        #
        #                 			where aj.type='cash' and ap.state='posted' and ap.payment_type='inbound' and aj.company_id = %s
        #                 			 and  to_char(date_trunc('day',ap.payment_date),'YYYY-MM-DD')
        #                 			 ::date between %s and %s group by pc.id
        #
        #
        #
        #
        #                             '''
        #
        #     self.env.cr.execute(query2, (company_id, date_start, date_end,))
        #     for row in self.env.cr.dictfetchall():
        #
        #         counters_id = (row['id'] if row['id'] else 0)
        #
        #         # for i in pos_config_ids:
        #         if config_id == counters_id:
        #             if row['payment_amount']:
        #                 cr+=row['payment_amount']
        #             else:
        #                 cr=0
        #             # cr += (row['payment_amount'] if row['payment_amount'] else 0.0)
        #         else:
        #             cr = 0
        #
        #     sessions_ids = self.env['pos.session'].search([
        #
        #         ('config_id', '=', config_id)])
        #     pos_order = self.env['pos.order'].search([('date_order', '>=', date_start), ('date_order', '<=', date_end),
        #                                               ('company_id', '=', company_id),
        #                                               ('state', 'in', ['paid', 'invoiced', 'done']),
        #                                               ('session_id', 'in', sessions_ids.ids)])
        #
        #     for inv in pos_order:
        #         config = self.env['pos.config'].browse(pos_config_ids)
        #         # for i in pos_config_ids:
        #         for pfg in config:
        #             if pfg.id == config_id:
        #                 for invoice in inv.invoice_id:
        #                     for i in invoice.payment_move_line_ids:
        #                         account_move_line = self.env['account.move.line'].search(
        #                             [('id', '=', i.id), ('journal_id.type', '=', 'cash'),('journal_id.name', '!=', 'Round off'),('name','!=','Customer Payment'),
        #                              ('move_id.state', '=', 'posted')])
        #                         for j in account_move_line:
        #                             cash += j.credit
        #
        #             # for h in inv.config_id:
        #
        #                 # for i in pos_config_ids:
        #
        #         # pos_account = self.env['account.bank.statement.line'].search([('id', 'in', inv.statement_ids.ids)])
        #         # for account in pos_account:
        #         #     if account.journal_id.type == 'cash':
        #         #         cash += account.amount
        #     # st_line_ids = self.env["account.bank.statement.line"].search(
        #     #     [('pos_statement_id', 'in', pos_order.ids)]).ids
        #     # if st_line_ids:
        #     #     self.env.cr.execute("""
        #     #                 SELECT COALESCE(sum(am.amount),'0') total
        #     #     FROM account_move as am,
        #     #     account_bank_statement_line AS absl,
        #     #          account_bank_statement AS abs,
        #     #          account_journal AS aj
        #     #     WHERE absl.statement_id = abs.id and am.statement_line_id=absl.id
        #     #         AND abs.journal_id = aj.id and aj.type ='cash'
        #     #         --and aj.debt != 'true'
        #     #         AND absl.id IN %s
        #     #             """, (tuple(st_line_ids),))
        #     #     cash = self.env.cr.dictfetchall()
        #     # else:
        #     #     cash = [{'total': 0.0}]
        #
        # res = {
        #     'cash': cash+cr+cm,
        # }

        # lines.append(res)

        # if res:
        #     return res
        # else:
        #     return []

    def get_card_wise(self, data, config_id):

        lines = []
        date_start = data['form']['date_start']
        date_end = data['form']['date_end']
        company_id = data['form']['company_id']
        counter_only = data['form']['counter_only']
        pos_config_ids = data['form']['pos_config_ids']
        card = 0
        cr=0
        cm=0

        if counter_only:


            re5='''
            
            
            select a.pos_config_ids as id,sum(apr.amount) as amr from account_partial_reconcile as apr

	left join account_move_line as aml on aml.id=apr.credit_move_id
	left join account_move as am on am.id=aml.move_id
	left join account_journal as aj on aj.id=am.journal_id
	left join account_account as aa on aml.account_id=aa.id
		left join account_invoice as a on (a.account_id=aa.id)




	where aj.company_id=%s and aj.type='bank' and aj.name<>'Round off' and a.partial_payment_remark is not null and a.state='paid'
	and a.pos_config_ids is not null 
            			 and  to_char(date_trunc('day',apr.create_date),'YYYY-MM-DD')
            			 ::date between %s and %s and aml.payment_id not in ( select ap.id from account_payment as ap
                                                			left join account_journal as aj on(aj.id=ap.journal_id)
                                                			left join pos_config as pc on(pc.id=ap.pos_counter_ids)

                                                			where aj.name<>'Round off' and ap.payment_type='inbound' and aj.company_id = %s
                                                			 and  to_char(date_trunc('day',ap.payment_date),'YYYY-MM-DD')
                                                			 ::date between %s and %s)

                                                			 group by a.pos_config_ids
            
            
            
            
            
            
            '''

            self.env.cr.execute(re5, (company_id, date_start, date_end,company_id, date_start, date_end,))
            for row in self.env.cr.dictfetchall():

                coun_id = (row['id'] if row['id'] else 0)

                # for i in pos_config_ids:
                if config_id == coun_id:
                    cm += (row['amr'] if row['amr'] else 0.0)
                else:
                    cm = 0

            query28 = '''

                                                select pc.id as id ,sum(ap.amount) as payment_amount from account_payment as ap
                                    			left join account_journal as aj on(aj.id=ap.journal_id)
                                    			left join pos_config as pc on(pc.id=ap.pos_counter_ids)

                                    			where aj.type='bank' and ap.state='posted' and ap.payment_type='inbound' and aj.company_id = %s
                                    			 and  to_char(date_trunc('day',ap.payment_date),'YYYY-MM-DD')
                                    			 ::date between %s and %s group by pc.id




                                                '''

            self.env.cr.execute(query28, (company_id, date_start, date_end,))
            for row in self.env.cr.dictfetchall():

                counters_id = (row['id'] if row['id'] else 0)

                # for i in pos_config_ids:
                if config_id == counters_id:
                    cr += (row['payment_amount'] if row['payment_amount'] else 0.0)
                else:
                    cr = 0



            sessions_ids = self.env['pos.session'].search([

                ('config_id', '=', config_id)])
            pos_order = self.env['pos.order'].search([('date_order', '>=', date_start), ('date_order', '<=', date_end),
                                                      ('company_id', '=', company_id),
                                                      ('state', 'in', ['paid', 'invoiced', 'done']),
                                                      ('session_id', 'in', sessions_ids.ids)])

            for inv in pos_order:
                config = self.env['pos.config'].browse(pos_config_ids)
                # for i in pos_config_ids:
                for pfg in config:
                    if pfg.id == config_id:
                        for invoice in inv.invoice_id:
                            for i in invoice.payment_move_line_ids:
                                account_move_line = self.env['account.move.line'].search(
                                    [('id', '=', i.id), ('journal_id.type', '=', 'bank'),
                                     ('move_id.state', '=', 'posted'),('journal_id.name', '!=', 'Round off'),('name','!=','Customer Payment')])
                                for j in account_move_line:
                                    card += j.credit

                        # for i in pos_config_ids:


        res = {
            'card': card+cr+cm,
        }

        # lines.append(res)

        if res:
            return res
        else:
            return []

    # def get_total_session(self,data, config_id):
    #     lines = []
    #     date_start = data['form']['date_start']
    #     date_end = data['form']['date_end']
    #     company_id = data['form']['company_id']
    #     counter_only = data['form']['counter_only']

    @api.model
    def render_html(self, docids, data=None):

        date_start = data['form']['date_start']
        date_end = data['form']['date_end']
        pos_config_ids = data['form']['pos_config_ids']
        config = self.env['pos.config'].browse(pos_config_ids)
        counter_only = data['form']['counter_only']
        header = False

        config.sorted(key=lambda r: r.id)
        count_of_config = 0
        # if counter_only:
        #     count_of_config =len(pos_config_ids)
        # if count_of_config ==1:
        #     header=self.env['pos.config'].browse(pos_config_ids).receipt_header

        valueone = 0
        valuetwo = 0
        valuethree = 0
        valuefour = 0
        valuefive = 0
        valuesix = 0

        date_object_date_start = datetime.strptime(date_start, '%Y-%m-%d').date()
        date_object_date_end = datetime.strptime(date_end, '%Y-%m-%d').date()
        rescash = {}
        rescard = {}
        rescashline = []
        rescardline = []
        rescreditline = []
        total_session = []

        if not counter_only:
            valueone = self.get_cash(data)
            valuetwo = self.get_card(data)
            valuethree = self.get_credit(data)

        if counter_only and pos_config_ids:

            valuethree = self.get_credit(data)
            for pfg in config:
                cash = self.get_cash_wise(data, pfg.id)
                card = self.get_card_wise(data, pfg.id)
                credit = self.get_credit_so(data, pfg.id)
                total = cash['cash'] + card['card']
                total_session.append(total)
                rescashline.append(self.get_cash_wise(data, pfg.id))
                rescardline.append(self.get_card_wise(data, pfg.id))
                rescreditline.append(self.get_credit_so(data, pfg.id))
                # rescreditline.append(self.get_card_so(data, pfg.id))
                # total_session.append(self.get_credit_so(data, pfg.id))

        valuefour = rescashline

        valuefive = rescardline

        valuesix = rescreditline
        total_session_wise = total_session

        docargs = {
            'doc_ids': self.ids,
            'date_start': date_object_date_start,
            'date_end': date_object_date_end,
            'data': data['form'],
            'pos_config_ids': pos_config_ids,
            'config': config,
            'time': time,
            'valueone': valueone,
            'valuetwo': valuetwo,
            'valuethree': valuethree,
            'counter_only': counter_only,
            'valuefour': valuefour,
            'valuefive': valuefive,
            'valuesix': valuesix,
            'total_session_wise': total_session_wise,
            'config_count': count_of_config,
            'header': header
        }

        return self.env['report'].render('qlty_salesreport_xls.qlty_salesreport_pdf', docargs)
