        ...
        get_won_points: function(){
            ...
            for (var i = 0; i < orderLines.length; i++) {
                ...
                for (var j = 0; j < rules.length; j++) {
                    var rule = rules[j];
                    total_points += (line.get_quantity() * rule.pp_product);
                    total_points += (line.get_price_with_tax() * rule.pp_currency);
...
//            if (this.get_total_with_tax() >= 100){
//                total_points += Math.floor(this.get_total_with_tax() * rounding_total);
//
//            }

//            total_points += Math.floor(this.get_total_with_tax() * rounding_total);

            total_points += ( total_sold * this.pos.loyalty.pp_currency);
            total_points += ( product_sold * this.pos.loyalty.pp_product);
            total_points +=( this.pos.loyalty.pp_order);
        ...
        get_spent_points: function() {
            if (!this.pos.loyalty || !this.get_client()) {
                ...
                for (var i = 0; i < lines.length; i++) {
                    ...
                    if (reward) {
                        if (reward.type === 'gift') {
                            points += (line.get_quantity() * reward.point_cost);
        ...
        get_new_points: function() {
            if (!this.pos.loyalty || !this.get_client()) {
                ...
                var t=this.get_won_points()
                var u= this.get_spent_points()
                var w =(this.get_won_points() - this.get_spent_points());
                return w
            }
        },
        ...
        get_new_total_points: function() {
            if (!this.pos.loyalty || !this.get_client()) {
                ...
                var y=this.get_client().loyalty_points
                var e= this.get_new_points()
                var q =(this.get_client().loyalty_points + this.get_new_points());
                return q
            }
        },
        ...
        get_spendable_points: function(){
            if (!this.pos.loyalty || !this.get_client()) {
                ...
                return (this.get_client().loyalty_points - this.get_spent_points());
            }
        },
        ...
        apply_reward: function(reward){
            ...
            if (!client) {
                ...
                var discount = (order_total * reward.discount);
                ...
                if ( ( spendable * product.price) > order_total ) {
                    spendable = ( Math.floor(order_total / product.price));
                }
