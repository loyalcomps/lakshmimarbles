...
class QltyCat(models.TransientModel):
    _name = "qlty.sale.purchase.xls"
    _description = "Quality Super Bazar Date wise Sale - Purchase report"
    ...
    def export_xls(self):
        ...
        if context.get('xls_export'):
            return {'type': 'ir.actions.report.xml',
                    'report_name': 'qlty_sale_purchase_xls.qlty_sale_purchase_xls.xlsx',
                    'datas': datas,
                    'name': 'Date Wise Sale- Purchase Wise Report'
                    }
    ...
    def export_pdf(self):
        ...
        datas['model'] = 'sale.order'
        datas['form'] = self.read()[0]
        report_name = 'qlty_sale_purchase_xls.salepurchase_report_pdf'
        for field in datas['form'].keys():
            if isinstance(datas['form'][field], tuple):
                datas['form'][field] = datas['form'][field][0]
