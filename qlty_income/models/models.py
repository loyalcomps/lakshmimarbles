...
class qlty_income(models.Model):
    _name = 'qlty.income'
    _description = " Income"
    ...
    name = fields.Char(string='Income Description', required=True)
    ...
    move_line_ids = fields.One2many('account.move.line', 'branchinm_id', copy=False, ondelete='restrict')
    ...
    def income_move(self):
        ...
        if self.product_id:
            ...
            if not account:
                raise UserError(
                    _("No INCOME account found for the product %s (or for it's category), please configure one.") % (
    ...
    def _create_payment_entry_current(self, amount):
        ...
        counterpart_aml_dict = self._get_shared_move_line_vals_current( credit,debit, amount_currency, move.id, False)
        ...
        if not self.currency_id != self.company_id.currency_id:
            amount_currency = 0
        liquidity_aml_dict = self._get_shared_move_line_vals2_current( debit,credit, -amount_currency, move.id, False)
...
class AcoountMoveLines(models.Model):
    ...
    branchinm_id = fields.Many2one('qlty.income', string=" Expense",
...
class product_expense(models.Model):
    ...
    income_bool = fields.Boolean(string="Income", default=False)
...
class product_expense(models.Model):
    ...
    income_bool = fields.Boolean(string="Income", related='product_tmpl_id.income_bool')
