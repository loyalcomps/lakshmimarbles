...
class StockReportsXls(ReportXlsx):
    ...
    def get_category(self, data):
        if data.get('form', False) and data['form'].get('category_id', False):
            l2 = []
            obj = self.env['pdct.category'].search([('id', 'in', data['form']['category_id'])])
            for j in obj:
                l2.append(j.id)
            return l2
        return ''
    ...
    def get_supplier(self, data):
        if data.get('form', False) and data['form'].get('supplier_id', False):
            l2 = []
            obj = self.env['account.invoice.line'].search([('invoice_id.partner_id', 'in', data['form']['supplier_id'])])
            for j in obj:
                if j.product_id.id not in l2:
                    l2.append(j.product_id.id)
            return l2
        return ''
    ...
    def get_lines(self, data, warehouse):
        ...
        suppl = self.get_supplier(data)
        if categ:
            stock_history = self.env['product.product'].search([('category_id', 'in', categ),('available_in_pos','=',True)])

        elif suppl:
            stock_history = self.env['product.product'].browse(suppl)

        else:
            stock_history = self.env['product.product'].search([('available_in_pos','=',True)])
        for obj in stock_history:
            sale_value = 0
            purchase_value = 0
            product = self.env['product.product'].browse(obj.id)
            sale_obj = self.env['pos.order.line'].search([('order_id.state', 'in', ('invoiced','paid', 'done')),
                                                           ('product_id', '=', product.id)
                                                           ])
            for i in sale_obj:
                sale_value = sale_value + i.qty
            purchase_obj = self.env['account.invoice.line'].search([('invoice_id.state', 'in', ('draft','open', 'paid')),
                                                                   ('product_id', '=', product.id),
                                                                    ('invoice_id.type', '=', 'in_invoice'),
                                                                   ('invoice_id.picking_type_id', '=', warehouse)])
            for i in purchase_obj:
                purchase_value = purchase_value + i.quantity
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        sheet.write(4, 0, 'BARCODE', format21)
        ...
        for i in get_warehouse[1]:
            ...
            for each in get_line:
                sheet.write(prod_row, prod_col, each['barcode'], font_size_8)
...
StockReportsXls('report.qlty_export_stockinfo_xls.qlty_stock_report_xls.xlsx', 'product.product')
