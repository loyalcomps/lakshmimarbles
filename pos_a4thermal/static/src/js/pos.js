...
var round_pr = utils.round_precision;
    ...
    models.PosModel = models.PosModel.extend({
        initialize: function (session, attributes) {
            ...
            partner_model.fields.push('qty_available','product_mrp','hsn');

            var res_partner_model = _.find(this.models, function(model){ return model.model === 'res.partner'; });
            res_partner_model.fields.push('x_gstin');
...
//                        product.qty_available -= line.get_quantity();
    ...
    models.Order = models.Order.extend({
      ...
      get_total_with_mrp: function() {
         var total_mrp=0;
          this.orderlines.each(function(line){
               total_mrp += line.get_product_mrp() * line.get_quantity();
          });
     ...
     // get_total_with_tax: function() {
         //   return Math.round(this.get_total_without_tax() + this.get_total_tax());
      //  },
    ...
    export_for_printing: function() {
     ...
     get_quantity_round: function(){
        return this.quantity.toFixed(2);
    },
     get_price_with_tax_round: function(){
        return this.get_all_prices().priceWithTax.toFixed(2);
    },
     get_price_without_tax_round: function(){
        return this.get_all_prices().priceWithoutTax.toFixed(2);
    },
     get_tax_round: function(){
        return this.get_all_prices().tax.toFixed(2);
    },

    get_taxes_name_fiscal_print: function(){
        var taxdetails = {};
        var taxes_ids = this.get_tax_details();
        for(var id in taxes_ids){
            taxdetails[id] = this.pos.taxes_by_id[id].name
        }
        return taxdetails;
