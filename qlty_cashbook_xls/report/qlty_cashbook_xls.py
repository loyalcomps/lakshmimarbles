...
class qltycashbookXls(ReportXlsx):
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        sheet = workbook.add_worksheet(_('CASHBOOK'))
        sheet.set_landscape()
        ...
        journal = self.env['account.journal'].browse(data['form']['journal_ids']).name
        ...
        blue_mark2 = workbook.add_format(
            {'bottom': True, 'top': True, 'right': True, 'left': True, 'font_size': 14, 'bold': True,
             'color': '000000', 'bg_color': '09b486', 'align': 'center'})

        # blue_mark2.set_bottom(1)
        # blue_mark2.set_top(1)
        # blue_mark2.set_right(1)
        # blue_mark2.set_left(1)
        ...
        sheet.merge_range('A1:E1', company, blue_mark3)
        sheet.merge_range('A2:E2', company_address if company_address  else '', blue_mark2)
        sheet.merge_range('A3:E3', "CASH Book Report", blue_mark2)
        sheet.merge_range('A4:E4', "Journal Name : " + journal, blue_mark2)
        sheet.merge_range('A5:E5', date_object_date_start.strftime('%d-%m-%Y') + '-' + date_object_date_end.strftime('%d-%m-%Y'), blue_mark2)
        ...
        sheet.write_number(linw_row, 2, abs(float(opening_bal)), font_size_8)
        sheet.write_number(linw_row, 3, 0, font_size_8)
        sheet.write(linw_row, 4, str(opening_bal) + ' DR' if opening_bal >= 0 else str(abs(opening_bal)) + ' CR', font_size_8)
        linw_row = 7
        line_column = 0
        ...
        for line in self.get_lines_finalpos(data):
            bal = bal + line['amount'] - line['expense']
            am = line['amount'] - line['expense']
            ...
            sheet.write(linw_row, line_column + 2, abs(float(am)), font_size_8)
            sheet.write_number(linw_row, line_column + 3, 0, font_size_8)
            sheet.write(linw_row, line_column + 4, str(bal) + ' DR' if bal >= 0 else str(abs(bal)) + ' CR', font_size_8)
            linw_row = linw_row + 1
            line_column = 0
        ...
        for line in self.get_lines_final(data):
            ...
            sheet.write(linw_row, line_column, line['date'], font_size_8)
            sheet.write(linw_row, line_column + 1, line['sname'] if line['sname'] else '/' + '' + line['particular'] , font_size_8)
            sheet.write(linw_row, line_column + 2, abs(line['amount']), font_size_8)
            sheet.write_number(linw_row, line_column + 3, abs(line['expense']), font_size_8)
            sheet.write(linw_row, line_column + 4, str(bal) + ' DR' if bal >= 0 else str(abs(bal)) + ' CR', font_size_8)
            linw_row = linw_row + 1
            line_column = 0
...
qltycashbookXls('report.qlty_cashbook_xls.cashbook_report_xls.xlsx', 'account.move')
