...
class product_pricelist(models.Model):
    ...
    def _get_product_id(self):
        ...
        if self.env.context.get('product_template_id', False):
            ...
            product_id = self.env.context.get('product_template_id')
        elif self.env.context.get('product_id', False):
            product = self.env['product.product'].browse(
                [self.env.context.get('product_id')]
            )
            if product:
                product_id=product[0].product_tmpl_id.id
    ...
    def _get_price_manual(self):
        ...
        if self._context.get('product_id'):
            product_template_id = self.env['product.product'].search(
                [('id', '=', self._context.get('product_id'))]).product_tmpl_id.id

            if self.env['pos.pricelist.items'].search(
                        [
                            ('pos_pricelist_id', '=', self.id),
