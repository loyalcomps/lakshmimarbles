...
//    }
    ...
    models.push(
        {
            ...
            fields: ['id', 'fixed_price', 'date_end', 'applied_on', 'min_quantity',
                'percent_price', 'date_start', 'product_tmpl_id', 'pos_pricelist_id', 'compute_price', 'categ_id','barcode_id'],
    ...
    pos_model.Order = pos_model.Order.extend({
        ...
        add_product: function (product, options) {
            ...
            if (pricelist_id && options && options.barcode_id){
                if (this.pos.get_client()){
                    self.apply__selected_pricelist(this.pos.get_client(), pricelist_id,options);
                }
            }
            if ((pricelist_id && !options) ||(pricelist_id && options && ! options.barcode_id) ){
                if (this.pos.get_client()){
        ...
        set_price: function (line, item) {
            ...
            switch (item.compute_price){
                ...
                case 'percentage':
//                      if (line.barcode_id){
//                        new_price = line.barcode_price -(line.barcode_price * item.percent_price / 100); break;
//                    }
//                    else{
//                        new_price = line.product.price -(line.product.price * item.percent_price / 100); break;
//                    }
                    if (line.barcode_id){
                        if (line.product_mrp <= (line.barcode_price -(line.barcode_price * item.percent_price / 100))){
                            new_price = line.product_mrp; break;
                            }
                        else{
                            new_price = line.barcode_price -(line.barcode_price * item.percent_price / 100); break;
                        }
//                        new_price = line.barcode_price -(line.barcode_price * item.percent_price / 100); break;
                        ...
                        if (line.product.product_mrp <=  (line.product.price -(line.product.price * item.percent_price / 100))){
                            new_price = line.product.product_mrp; break;
                            }
                        else{
                            new_price =  line.product.price -(line.product.price * item.percent_price / 100); break;
                        }
//                        new_price = line.product.price -(line.product.price * item.percent_price / 100); break;
        ...
        // this function returns the new price after checking the computation method of the pricelist item if pricelist is already selected
        set_price_selected: function (line, item,options) {
            var new_price = 0;
            switch (item.compute_price){
                case 'fixed': new_price = item.fixed_price; break;
                case 'percentage':
                    ...
                    if (options.barcode_id){
                        if (options.product_mrp <= (options.barcode_price -(options.barcode_price * item.percent_price / 100))){
                            new_price = options.product_mrp; break;
                            }
                        else{
                            new_price = options.barcode_price -(options.barcode_price * item.percent_price / 100); break;
                        }
//                        new_price = options.barcode_price -(options.barcode_price * item.percent_price / 100); break;
                    }
                    else{
...
//                        new_price = line.product.price -(line.product.price * item.percent_price / 100); break;
        ...
        //apply price list when pricelist has been already selected
        apply__selected_pricelist: function(client, pricelist_id,options){
            var self = this;
...
//            checking the start date and end date of these items and selecting only the valid items
            ...
            //looping through each line
                ...
                var product_item = self.find_pricelist_item(lines[total_lines-1].product.product_tmpl_id, product_ids);
                // checking if this product's category has a valid pricelist rule set or not
                var categ_item = self.find_pricelist_item(lines[total_lines-1].product.categ_id[0], category_ids);
                var temp = -1;
                var new_price = lines[total_lines-1].product.price;
//                checking if the product has any pricelist item set
                if(product_item){
                    for(var j in product_items){
                        if (options.barcode_id){
                            if(product_items[j].product_tmpl_id[0] == lines[total_lines-1].product.product_tmpl_id &&
                            product_items[j].barcode_id[0] == options.barcode_id){

                                if(lines[total_lines-1].quantity >= product_items[j].min_quantity){
                                    if(temp < 0){
                                        temp = lines[total_lines-1].quantity - product_items[j].min_quantity;
                                        new_price = self.set_price_selected(lines[total_lines-1], product_items[j],options);
                                    }
                                    else if(temp > (lines[total_lines-1].quantity - product_items[j].min_quantity) &&
                                        (lines[total_lines-1].quantity - product_items[j].min_quantity) >= 0){
                                        temp = lines[total_lines-1].quantity - product_items[j].min_quantity;
                                        new_price = self.set_price_selected(lines[total_lines-1], product_items[j],options);
                                    }
                                }
                    ...
                    lines[total_lines-1].set_unit_price(new_price);
                }
                else if(categ_item){
                    for(var j in category_items){
                        if(category_items[j].categ_id[0] == lines[total_lines-1].product.categ_id[0]){
                           if(lines[total_lines-1].quantity >= category_items[j].min_quantity)
                            {
                                if(temp < 0){
                                    temp = lines[total_lines-1].quantity - category_items[j].min_quantity;
                                    new_price = self.set_price_selected(lines[total_lines-1], category_items[j],options);
                                }
                                else if(temp > (lines[total_lines-1].quantity - category_items[j].min_quantity) &&
                                    (lines[total_lines-1].quantity - category_items[j].min_quantity) >= 0){
                                    temp = lines[total_lines-1].quantity - category_items[j].min_quantity;
                                    new_price = self.set_price_selected(lines[total_lines-1], category_items[j],options);
                                }
                            }
...
//                if there are no rules set for product or category, we will check global pricelists
                    ...
                    for(var j in global_items){
                        if(lines[total_lines-1].quantity >= global_items[j].min_quantity)
                        {
                            if(temp < 0){
                                temp = lines[total_lines-1].quantity - global_items[j].min_quantity;
                                new_price = self.set_price_selected(lines[total_lines-1], global_items[j],options);
                            }
                            else if(temp > (lines[total_lines-1].quantity - global_items[j].min_quantity) &&
                                (lines[total_lines-1].quantity - global_items[j].min_quantity) >= 0){
                                temp = lines[total_lines-1].quantity - global_items[j].min_quantity;
                                new_price = self.set_price_selected(lines[total_lines-1], global_items[j],options);
                            }
                        }
...
//                else we set the original price
                    ...
                    if (options.barcode_id){
                        lines[total_lines-1].set_unit_price(options.barcode_price);
                    }
                    else{
                        lines[total_lines-1].set_unit_price(lines[total_lines-1].product.price);
...
//                checking if the product has any pricelist item set
                if(product_item){
                    for(var j in product_items){
                        if (lines[l].barcode_id){
                            if(product_items[j].product_tmpl_id[0] == lines[l].product.product_tmpl_id &&
                            product_items[j].barcode_id[0] == lines[l].barcode_id){
                            ...
                            if(product_items[j].product_tmpl_id[0] == lines[l].product.product_tmpl_id && ! product_items[j].barcode_id){
...
//                else we set the original price
                    ...
                    if (lines[l].barcode_id){
                        lines[l].set_unit_price(lines[l].barcode_price);
                    }
                    else{
