...
class AccountInvoiceLine(models.Model):
    ...
    @api.depends('price_subtotal_taxinc','quantity','invoice_id.freight_pdct','product_id','spcl_disc')
    def get_landing_cost(self):

        for line in self:
            landing_cost = 0

            landing_costper_quant = line.price_subtotal_taxinc/line.quantity if line.quantity != 0 else 0
            unit_spcl_disc = line.spcl_disc/line.quantity if line.quantity !=0 else 0
            landing_cost = landing_costper_quant+line.invoice_id.freight_pdct-unit_spcl_disc
            line.landing_cost = landing_cost
    ...
    @api.depends('price_unit', 'discount', 'invoice_line_tax_ids', 'quantity', 'product_id', 'invoice_id.currency_id',
                 'invoice_id.partner_id', 'invoice_id.company_id', 'invoice_id.discount','invoice_id.discount_type','inclusive')
    ...
    def create(self, vals):
        ...
        if vals['product_id'] :
            ...
            if 'category_id' in vals:
                ...
                res['categ_id'] = vals['category_id']
            if 'barcode' in vals:
                # product_id.barcode = vals['barcode']
                res['barcode'] = vals['barcode']
            if 'expiry_date' in vals:
                # product_id.expiry_date = vals['expiry_date']
                res['expiry_date'] = vals['expiry_date']
...
class AccountInvoice(models.Model):
    ...
    amount_freight = fields.Monetary(string="Freight Rate", store=True, readonly=True, track_visibility='always',
                                     compute="_compute_amount")
    freight_pdct = fields.Float(string="Freight Charge", default=0.0,compute="_onchange_freight")
    ...
    @api.onchange('freight','invoice_line_ids.quantity' )
        ...
        for record in self:
            if record.type in ['in_invoice']:
                for line in record.invoice_line_ids:
                    qty+=line.quantity
                if qty != 0:
                    record.freight_pdct = record.freight/qty
    ...
    @api.depends('invoice_line_ids.price_subtotal', 'tax_line_ids.amount', 'currency_id', 'company_id', 'date_invoice',
                 'discount','type','round_off_value','round_off_operation','freight','inclusive')
            ...
            if inv.payment_term_id:
                ...
                for i, t in enumerate(totlines):
                    ...
                    if self.round_off_value != 0.0 and self.type == 'in_invoice':
                        iml.append({
                            'type': 'dest',
                            'name': name,
                            'price': t[1] - self.rounded_total+self.discount_amt-self.freight,
                ...
                if self.round_off_value != 0.0 and self.type == 'in_invoice':
                    iml.append({
                        'type': 'dest',
                        'name': name,
                        'price': total - self.rounded_total+self.discount_amt-self.freight,
                        'account_id': inv.account_id.id,
