...
class QltyCat(ReportXlsx):
    ...
    def get_lines(self, data):
        ...
        if category_id:
            sale_lines = self.env["pos.order.line"].search([('order_id.date_order', '>=', date_start),('order_id.date_order', '<=', date_end),('company_id', '=', company_id),('product_id.category_id', '=', category_id),
            ...
            purchase_lines = self.env["account.invoice.line"].search(
                    [('invoice_id.date_invoice', '>=', date_start),('invoice_id.date_invoice', '<=', date_end),('company_id', '=', company_id),
                    ('product_id.category_id', '=', category_id),
                     ('invoice_id.type','=','in_invoice'),
                     ('invoice_id.state','in',['open','paid'])])
            category = self.env["pdct.category"].search([('id','=',category_id)])
        else:
            sale_lines = self.env["pos.order.line"].search(
                    [('order_id.date_order', '>=', date_start),('order_id.date_order', '<=', date_end),('company_id', '=', company_id),
                     ('order_id.state', 'in', ['done', 'paid', 'invoiced'])])
            out_invoice_lines = self.env["account.invoice.line"].search(
                    [('invoice_id.date_invoice', '>=', date_start), ('invoice_id.date_invoice', '<=', date_end),
                     ('invoice_id.state', 'in', ['open', 'paid'])
            ...
            purchase_lines = self.env["account.invoice.line"].search([('invoice_id.type','=','in_invoice'),
                    ('invoice_id.date_invoice', '>=', date_start),('invoice_id.date_invoice', '<=', date_end), ('company_id', '=', company_id),('invoice_id.state','in',['open','paid']) ])
            category = self.env["pdct.category"].search([])
        ...
        for i in category:
            ...
            for j in sale_lines:
                if i.id == j.product_id.category_id.id:
                    s_amt += j.price_subtotal_incl
                    s_qty += j.qty

            for k in purchase_lines:
                if i.id == k.product_id.category_id.id:
                    p_amt += k.price_subtotal_taxinc
                    p_qty += k.quantity
            for l in out_invoice_lines:
                if i.id == l.product_id.category_id.id:
                    s_amt += l.price_subtotal_taxinc
                    s_qty += l.quantity
            average_cost = (p_amt / p_qty) if p_qty != 0 else 0
            res = {
                'sl_no': sl,
                'category': i.name,
                's_qty': s_qty,
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        sheet.write('B3', "Category", blue_mark)
        sheet.write('C3', "Purchased Qty", blue_mark)
        sheet.write('D3', "Purchase Amount", blue_mark)
        sheet.write('E3', "Saled Qty", blue_mark)
        sheet.write('F3', "Sale Amount", blue_mark)
        sheet.write('G3', "Profit", blue_mark)
        # sheet.write('H3', "Onhand", blue_mark)
        # sheet.write('I3', "", blue_mark)
        ...
        for line in self.get_lines(data):
            ...
            sheet.write(linw_row, line_column + 2, line['p_qty'], font_size_8)
...
QltyCat('report.qlty_category_xls.qlty_category_xls.xlsx', 'sale.order')
