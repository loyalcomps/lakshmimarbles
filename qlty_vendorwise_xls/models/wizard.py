...
class Qltyvendor(models.TransientModel):
    _name = "qlty.vendorwise.xls"
    ...
    def export_xls(self):
        ...
        if context.get('xls_export'):
            return {'type': 'ir.actions.report.xml',
                    'report_name': 'qlty_vendorwise_xls.qlty_vendorwise_xls.xlsx',
                    'datas': datas,
    ...
    def export_pdf(self):
        ...
        datas['model'] = 'sale.order'
        datas['form'] = self.read()[0]
        report_name =  'qlty_vendorwise_xls.vendor_report_pdf'
        for field in datas['form'].keys():
            if isinstance(datas['form'][field], tuple):
                datas['form'][field] = datas['form'][field][0]
