# -*- coding: utf-8 -*-
from odoo import models, fields, tools, api
import odoo.addons.decimal_precision as dp

class product_notequaltax_compare(models.Model):
    _name = 'report.product.notequaltax.compare'

    _auto = False
    rec_name = 'proname'
    # _order = 'date desc'

    product_id = fields.Many2one(
        'product.product', string='Product Variant', readonly=True)

    proname =  fields.Char('Name')

    available_in_pos = fields.Boolean(string='Available In POS',readonly = True)
    barcode = fields.Char(string='Barcode', readonly=True)
    vendortax = fields.Char(string='Vendor Tax', readonly=True)
    customertax = fields.Char(string='Customer Tax', readonly=True)

    # WARNING : this code doesn't handle uom conversion for the moment
    def _product_notequaltax_select(self):
        select = """SELECT pt.id as id,pp.id as product_id,pt.name as proname,pt.available_in_pos as available_in_pos,
pp.barcode as barcode,
act.name as vendortax,apt.name as customertax
FROM product_supplier_taxes_rel ps
LEFT JOIN product_template pt
            ON pt.id = ps.prod_id
LEFT JOIN product_product pp 
            ON pt.id = pp.product_tmpl_id
LEFT JOIN product_taxes_rel ptr
			ON pt.id = ptr.prod_id
LEFT JOIN account_tax act
            ON act.id = ps.tax_id
LEFT JOIN account_tax apt
			ON apt.id = ptr.tax_id
WHERE act.name != apt.name     
"""
        return select

    def init(self):
        # tools.drop_view_if_exists(self._cr, self._table)
        tools.drop_view_if_exists(self._cr, 'report_product_notequaltax_compare')
        self._cr.execute("CREATE OR REPLACE VIEW report_product_notequaltax_compare AS (%s)" % (self._product_notequaltax_select()))


